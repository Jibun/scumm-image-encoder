...
class DecoderV3(common.ImageDecoderBase):
    def fixPalette(self):
    ...
    def decodeImage(self, lflf_path, image_path, palette_num):
        """ V3 background images share "character map" data with the objects, so decode all objects as well."""
    ...
    def decodeObjectImages(self, lflf_path, bitmap_path, image_path, pal_data):
        ...
        for objNum in objNumStrs:
            logging.info("Decoding v3 object #%s" % objNum)
            ...
            obj_image_path = "%s-OIv3_%s%s" % (ip, objNum, ipext)
            self.saveImage(obj_image_path, width, height, bmp_data, pal_data)
            obj_mask_path = "%s-OIv3_%s.mask%s" % (ip, objNum, ipext)
    ...
    def readObjectBitmap(self, bitmap_path, width, height, pal_data):
        ...
        try:
            img_data, mask_data = ega.decodeV3ObjectBitmap(bitmapFile, width, height)
    ...
    def readBitmap(self, bitmap_path, width, height, pal_data):
        bitmapFile = file(bitmap_path, 'rb')
        try:
            img_data = ega.decodeV3Bitmap(bitmapFile, width, height)
            return img_data
        finally:
            bitmapFile.close()
    ...
    def readMask(self, bitmap_path, width, height):
        bitmapFile = file(bitmap_path, 'rb')
        try:
            mask_data = ega.decodeV3Mask(bitmapFile, width, height)
            return mask_data
        finally:
            bitmapFile.close()
